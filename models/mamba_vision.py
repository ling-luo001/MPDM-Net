...
class Block_mamba(nn.Module):
    def __init__(self,
        ...
        self.forward_B_mamba = MambaVisionMixer(d_model=dim,
                                      d_state=16,
                                      d_conv=4,
                                      expand=4
                                      )
        ...
        self.backward_B_mamba = MambaVisionMixer(d_model=dim,
                                                d_state=16,
                                                d_conv=4,
                                                expand=4
                                                )
    ...
    def forward(self, x):
        # x = x + self.drop_path(self.gamma_1 * self.B_mamba(self.norm1(x)))
        x_forward, x_backward = x.clone(), torch.flip(x, [1])

        x_forward = x_forward + self.forward_B_mamba(x_forward)

        x_backward = x_backward + self.backward_B_mamba(x_backward)

        x = torch.cat([x_forward, torch.flip(x_backward,[1])], -1)

        # x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
        return x


class TF_mamba_attention(nn.Module):
    """
    Temporal-Frequency Hybrid Block with Attention and Mamba

    Attributes:
    cfg (Config): Configuration for the block.
    time_block (Block_Attention): Attention block for temporal dimension.
    freq_block (Block_mamba): Mamba block for frequency dimension.
    tlinear (Conv1d): Optional Conv1d layer for temporal dimension.
    flinear (Conv1d): Optional Conv1d layer for frequency dimension.
    """

    def __init__(self, cfg, inchannels, num_heads=4):  # 新增num_heads参数
        super(TF_mamba_attention, self).__init__()
        self.cfg = cfg
        self.hid_feature = inchannels

        # 频域使用Mamba
        self.time_block_mamba = Block_mamba(
        ...
        self.freq_block_mamba = Block_mamba(
            dim=self.hid_feature,
            mlp_ratio=4.0,  # 保持与原配置一致
            drop=0,
            drop_path=0,
        ...
        # # 维度调整层（保持与原结构兼容）
        # self.tlinear = nn.Conv1d(self.hid_feature, self.hid_feature, 1)  # 简化为1x1卷积
        # self.flinear = nn.Conv1d(self.hid_feature, self.hid_feature, 1)  # 输入输出通道保持一致

    def forward(self, x):
        """
        前向传播流程说明：
        1. 时域处理：将特征重组为(batch*freq, time, channel)后通过注意力模块
        2. 频域处理：将特征重组为(batch*time, freq, channel)后通过Mamba模块
        3. 使用残差连接保持信息流
        """
        B, C, T, F = x.size()  # 原始维度: (batch, channel, time, freq)

        # 第一步: 时域Mamba处理 --------------------------------------
        # 重组维度: (B, C, T, F) -> (B*F, T, C)
        x_mamba_time = x.permute(0, 3, 2, 1).contiguous().view(B * F, T, C)
        x_mamba_time = self.tlinear(self.time_block_mamba(x_mamba_time).permute(0,2,1)).permute(0,2,1) + x_mamba_time  # 残差连接
        # 恢复形状: (B*F, T, C) -> (B, F, T, C)
        x_mamba_time = x_mamba_time.view(B, F, T, C)

        # 第二步: 频域Mamba处理 -------------------------------------
        # 重组维度: (B, F, T, C) -> (B*T, F, C)
        x_mamba_freq = x_mamba_time.permute(0, 2, 1, 3).contiguous().view(B * T, F, C)
        # x_mamba_freq = self.freq_block_mamba(x_mamba_freq) + x_mamba_freq
        x_mamba_freq = self.flinear(self.freq_block_mamba(x_mamba_freq).permute(0,2,1)).permute(0,2,1) + x_mamba_freq  # 残差连接

        # 恢复形状: (B*T, F, C) -> (B, T, F, C)
        x_mamba_freq = x_mamba_freq.view(B, T, F, C)

        # 第三步: 时域Attention处理 ---------------------------------
        # 重组维度: (B, T, F, C) -> (B*F, T, C)
        x_attn_time = x_mamba_freq.permute(0, 2, 1, 3).contiguous().view(B * F, T, C)
        x_attn_time = self.time_block_attention(x_attn_time) + x_attn_time
        # 恢复形状: (B*F, T, C) -> (B, F, T, C)
        x_attn_time = x_attn_time.view(B, F, T, C)

        # 第四步: 频域Attention处理 ---------------------------------
        # 重组维度: (B, F, T, C) -> (B*T, F, C)
        x_attn_freq = x_attn_time.permute(0, 2, 1, 3).contiguous().view(B * T, F, C)
        x_attn_freq = self.freq_block_attention(x_attn_freq) + x_attn_freq
        # 恢复形状: (B*T, F, C) -> (B, T, F, C)
        x_attn_freq = x_attn_freq.view(B, T, F, C)

        # 最终维度调整: (B, T, F, C) -> (B, C, T, F)

        return x_attn_freq.permute(0, 3, 1, 2).contiguous()
