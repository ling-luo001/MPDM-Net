...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-rd_asymmetric_polar_demand_v2_full_seed1234}"
CONFIG_PATH="${CONFIG_PATH:-recipes/RD-Asymmetric-Polar-Demand-V2/RD-Asymmetric-Polar-Demand-V2.yaml}"
EPOCHS="${EPOCHS:-200}"
...
if [[ -z "${EXP_ROOT:-}" ]]; then
    if mountpoint -q /media/lz/WZZ; then
        EXP_ROOT="/media/lz/WZZ/mpdm_full_experiments/rd_asymmetric_polar_demand_v2"
    else
        EXP_ROOT="${HOME}/mpdm_full_experiments/rd_asymmetric_polar_demand_v2"
    fi
fi
...
if [[ -n "${matching_pid}" ]]; then
    printf '%s is already running as PID %s (pid file was missing).\n' \
...
printf 'Existing complete checkpoints in that directory are resumed automatically.\n'
